...
async function printInvoice(invoice) {
  try {
    ...
    const created = sale.created_at ? new Date(sale.created_at).toLocaleString("en-GB", {
      ...
      <body>
        <div class="invoice">
          ...
          <div class="meta">
            ...
            <div><b>Date & Time</b><span>${esc(created)}</span></div>
          ...
          <div class="totals">
            ...
            <div class="line"><span>Discount</span><strong>− ${money(sale.discount || 0)}</strong></div>
            <div class="grand"><span>Grand Total</span><span>${money(sale.total || 0)}</span></div>
          </div>
