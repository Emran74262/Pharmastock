...
function renderCart() {
  ...
  const discountPercent = Math.min(100, Math.max(0, Number($('discount')?.value || 0)));
  const discountAmount = subtotal * discountPercent / 100;
  const total = Math.max(0, subtotal - discountAmount);

  if ($('cartSubtotal')) $('cartSubtotal').textContent = money(subtotal);
  if ($('cartDiscount')) $('cartDiscount').textContent = money(discountAmount);
...
async function completeSale() {
  ...
  try {
    ...
    if (!r.ok) {
      $('saleMsg').innerHTML = `<div class="msg error">${esc(j.error || 'Sale failed.')}</div>`;
      return;
    }
    ...
    $('saleMsg').innerHTML = `
      <div class="msg">
        ...
        Invoice: <b>${esc(j.invoice)}</b> — ${money(j.total)}
        <br><br>
        <button onclick="printInvoice('${esc(j.invoice)}')">🖨️ Print Invoice</button>
      </div>`;
    ...
    $('saleMsg').innerHTML = '<div class="msg error">Server error while completing sale.</div>';
    console.error(e);
  }
}
...
document.addEventListener('input', function(e) {
  if (e.target && e.target.id === 'discount') updateSaleTotals();
});
